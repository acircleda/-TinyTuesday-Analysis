...
tweets %>%
  select(screen_name, created_at, text, favorite_count, retweet_count, is_retweet, ext_media_url) %>%
  filter(!is_retweet) %>% 
...
#subset of data
subset_tweets <- tweets %>% select(screen_name, created_at, text, favorite_count, retweet_count, is_retweet, ext_media_url_copy, ext_media_url_1, ext_media_url_2, ext_media_url_3, ext_media_url_4) %>%
  mutate(
    date = as_date(created_at),
    week = week(date),
...
# #random sample for content analysis of retweets

rewteets <- tweets %>%
  select(text, is_quote) %>%
  filter(is_quote == "TRUE") %>%
  select(text)

samples <- bind_rows(replicate(10, rewteets %>% sample_n(50), simplify=F), .id="Sample")
samples$Sample <- as.numeric(samples$Sample)



samples %>% filter(Sample == 1) %>% write.csv("50-random-retweets-1.csv")
samples %>% filter(Sample == 2) %>% write.csv("50-random-retweets-2.csv")
samples %>% filter(Sample == 3) %>% write.csv("50-random-retweets-3.csv")
samples %>% filter(Sample == 4) %>% write.csv("50-random-retweets-4.csv")
samples %>% filter(Sample == 5) %>% write.csv("50-random-tweets-5.csv")
test
